...
from nprach_repro.nprach_simulation import run_detection_example, save_json_result, save_probability_plot
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a moderate NPRACH detection-vs-SNR experiment.")
    parser.add_argument("--trials-per-snr", type=int, default=200, help="Number of signal-present transmissions per SNR point.")
...
def main() -> None:
    ...
    channel_cfg = ChannelConfig(profile="awgn", cfo_hz=200.0, num_rx_antennas=2, signal_present=True)
    ...
    json_path = save_json_result(_tagged_output_path(f"detection_example_result{output_suffix}", ".json"), result)
    plot_path = save_probability_plot(
        x_values=snr_db_list,
        y_values=[point.detection_probability for point in result.points],
        output_path=_tagged_output_path(f"detection_probability_vs_snr{output_suffix}", ".png"),
        title=f"NPRACH Detection Probability ({args.trials_per_snr} trials/SNR, {threshold_metadata['threshold_input_mode']})",
        y_label="Correct Detection Probability",
    )
    ...
    for point in result.points:
        print(
            ...
            f"correct={point.correct_detection_count}/{point.num_transmissions}, "
            f"p_detection={point.detection_probability:.3f}"
        )
